...
class CheckPage extends StatefulWidget {
 final String userId; // 👈 นี่คือ ID ของ Lecturer (actor_user_id)
...
class _CheckPageState extends State<CheckPage> {
 ...
 Future<void> _updateBookingStatus(String newStatus) async {
  ...
  try {
   // ⭐️ [แก้ไข] เพิ่ม 'actor_user_id' เข้าไปใน body
   final body = json.encode({
    'new_status': newStatus,
    'actor_user_id': widget.userId // 👈 เพิ่ม ID ของ Lecturer ที่กดปุ่ม
   });

   final response = await http.patch(
    url,
    headers: {'Content-Type': 'application/json'},
    body: body, // 👈 ใช้ body ตัวใหม่
   );
   // 🔼🔼🔼 ------------------- 🔼🔼🔼
   ...
   if (response.statusCode == 200) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
     SnackBar(
      // [แก้ไข] แสดง Error ที่ server ส่งมา
